...
times = np.arange(len(orig_data)) / sfreq_ori

# 为了看图清晰，只取前 5 秒（可改）
plot_sec = 5
plot_n_samples_ori = int(plot_sec * sfreq_ori)
plot_n_samples_pro = int(plot_sec * sfreq_pro)

orig_plot = orig_data[:plot_n_samples_ori]
proc_plot = proc_data[:plot_n_samples_pro]
times_plot_ori = times[:plot_n_samples_ori]
times_plot_pro = times[:plot_n_samples_pro]
# ==============================
# 3. 创建大图：3行2列对比
# ==============================
...
# 第一行：时域波形对比
# ==============================
axes[0,0].plot(times_plot_ori, orig_plot, color='#1f77b4', linewidth=0.8)
axes[0,0].set_title('原始 LFP 时域波形', fontsize=12)
axes[0,0].set_ylabel('幅值 (V)', fontsize=10)
axes[0,0].grid(alpha=0.3)

axes[0,1].plot(times_plot_pro, proc_plot, color='#ff4b5c', linewidth=0.8)
...
# 第二行：PSD 功率谱密度对比
# ==============================
f_orig, pxx_orig = welch(orig_data, sfreq_ori, nperseg=sfreq_ori*2)
f_proc, pxx_proc = welch(proc_data, sfreq_pro, nperseg=sfreq_pro*2)
...
tfr_o = tfr_array_morlet(orig_s, sfreq=sfreq_ori, freqs=freqs, n_cycles=3, output='power')[0,0]

# 处理后时频图
proc_s = proc_plot[None, None, :]
tfr_p = tfr_array_morlet(proc_s, sfreq=sfreq_pro, freqs=freqs, n_cycles=3, output='power')[0,0]

axes[2,0].imshow(tfr_o, aspect='auto', origin='lower',
                 extent=[times_plot_ori.min(), times_plot_ori.max(), freqs.min(), freqs.max()], cmap='jet')
axes[2,0].set_title('原始 时频图')
axes[2,0].set_ylabel('Freq (Hz)')

axes[2,1].imshow(tfr_p, aspect='auto', origin='lower',
                 extent=[times_plot_pro.min(), times_plot_pro.max(), freqs.min(), freqs.max()], cmap='jet')
